...
import org.nghru_uk.ghru.jobs.GcmJobService
import org.nghru_uk.ghru.jobs.JobManagerFactory
import org.nghru_uk.ghru.jobs.SchedulerJobService
import org.nghru_uk.ghru.sync.*
import org.nghru_uk.ghru.util.LiveDataCallAdapterFactory
...
import org.nghru_uk.ghru.api.AuthAPIService
...
class AppModule {
    ...
    fun provideNghruService(tokenManager: TokenManager): NghruService {
        return makeRetrofit(accessTokenProvidingInterceptor(tokenManager), authenticator = tokenAuthenticator(tokenManager))
            .create(NghruService::class.java)
    }


    fun makeRetrofit(vararg interceptors: Interceptor, authenticator: Authenticator) = Retrofit.Builder()
        .baseUrl(Prefs.getString("Ipaddress", BuildConfig.SERVER_URL))
        .client(makeHttpClient(interceptors, authenticator))
        .addConverters()
        .build()
    ...
    private fun makeHttpClient(interceptors: Array<out Interceptor>, authenticator: Authenticator) = OkHttpClient.Builder()
    ...
    private fun tokenAuthenticator(tokenManager: TokenManager) = Authenticator { _, response ->
        val authService = AuthAPIService()

        val refreshToken = tokenManager.getToken().refreshToken
        if (refreshToken == null) {
            null
        }
        else {
            val newAccessTokenResponse = authService.refreshToken(RefreshToken(refresh_token = refreshToken!!))
    ...
    @Singleton
    @Provides
    fun provideQuestionnaireSelfDao(db: NGRHUDb): QuestionnaireSelfDao {
        return db.questionnaireSelfDao()
    }
...
//    }
    ...
    @Singleton
    @Provides
    fun provideBloodPressureItemRequestDaoNew(db: NGRHUDb): BloodPresureItemRequestDaoNew {
        return db.bloodPressureItemRequestDaoNew()
    }

    @Singleton
    @Provides
    fun provideBloodPressureRequestDaoNew(db: NGRHUDb): BloodPresureRequestDaoNew {
        return db.bloodPressureRequestDaoNew()
    }
    ...
    @Provides
    internal fun provideBloodPressureMetaRequestLifecycleObserverNew(bloodPressureMetaRequestDaoNew: BloodPresureRequestDaoNew): BloodPressureMetaRequestLifecycleObserverNew {
        return BloodPressureMetaRequestLifecycleObserverNew(bloodPressureMetaRequestDaoNew)
    }
