package org.nghru_uk.ghru.ui.consent.getassetdialog
...
import org.nghru_uk.ghru.databinding.GetAssetDialogFragmentBinding
...
class GetAssetDialogFragment : DialogFragment(), Injectable {

    val TAG = GetAssetDialogFragment::class.java.getSimpleName()

    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory


    var binding by autoCleared<GetAssetDialogFragmentBinding>()
    var dataBindingComponent: DataBindingComponent = FragmentDataBindingComponent(this)
    @Inject
    lateinit var confirmationdialogViewModel: GetAssetDialogViewModel
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<GetAssetDialogFragmentBinding>(
            inflater,
            R.layout.get_asset_dialog_fragment,
            container,
            false
        )
        binding = dataBinding
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        if (consentCount == 1)
        {
            binding.msgTextView.setText(getString(R.string.one_consent_exist_msg))
        }
        else if (consentCount!! >= 2)
        {
            binding.msgTextView.setText(getString(R.string.two_consent_exit_msg_part_1) + " " + consentCount + " "+ getString(R.string.two_consent_exit_msg_part_2))
        }

        binding.continueButton.singleClick {

            dismiss()
            val bundle = bundleOf("Participant" to participant, "ConsentCount" to consentCount)
