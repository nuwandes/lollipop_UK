package org.nghru_uk.ghru.ui.activitytracker.scanbarcode.manualentry
...
import org.nghru_uk.ghru.databinding.ActivityTrackerPatientBarcodeManualentryFragmentBinding
import org.nghru_uk.ghru.di.Injectable
import org.nghru_uk.ghru.event.StationCheckRxBus
import org.nghru_uk.ghru.ui.activitytracker.scanbarcode.ScanBarcodeViewModel
...
class ManualEntryBarcodeFragment : Fragment(), Injectable {
    ...
    var binding by autoCleared<ActivityTrackerPatientBarcodeManualentryFragmentBinding>()
            ...
            StationCheckRxBus.getInstance().toObservable()
                .subscribe({ result ->
                    ...
                    Navigation.findNavController(activity!!, R.id.container)
                        .navigate(R.id.action_ManualEntryBarcodeFragment_to_ActivityTackeFragment, bundle)
                }, { error ->
                    print(error)
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<ActivityTrackerPatientBarcodeManualentryFragmentBinding>(
            inflater,
            R.layout.activity_tracker_patient_barcode_manualentry_fragment,
            container,
            false
        )
        binding = dataBinding
            ...
            if (participantResource?.status == Status.SUCCESS) {
                ...
                if (!participantResource.data?.stationStatus!!) {
                    ...
                    findNavController().navigate(
                        R.id.action_ManualEntryBarcodeFragment_to_ActivityTackeFragment,
                        bundle
                    )
                } else {
...
//                participantRequest?.meta = meta
//                val bundle = bundleOf("ParticipantRequest" to participantRequest)
//                findNavController().navigate(R.id.action_ManualEntryBarcodeFragment_to_ActivityTackeFragment, bundle)
//            } else if (participantResource?.status == Status.ERROR) {
//
//                val errorDialogFragment = ErrorDialogFragment()
