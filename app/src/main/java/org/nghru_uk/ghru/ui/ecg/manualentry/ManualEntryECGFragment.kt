package org.nghru_uk.ghru.ui.ecg.manualentry
...
import org.nghru_uk.ghru.databinding.EcgMaualBarcodeFragmentBinding
...
class ManualEntryECGFragment : Fragment(), Injectable {

    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory

    var dataBindingComponent: DataBindingComponent = FragmentDataBindingComponent(this)
    var binding by autoCleared<EcgMaualBarcodeFragmentBinding>()

    @Inject
    lateinit var viewModel: ManualEntryECGModel
            ...
            StationCheckRxBus.getInstance().toObservable()
                .subscribe({ result ->
                    ...
                    Navigation.findNavController(activity!!, R.id.container)
                        .navigate(R.id.action_manualScanFragment_to_guideMainFragment, bundle)
                }, { error ->
                    print(error)
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<EcgMaualBarcodeFragmentBinding>(
            inflater,
            R.layout.ecg_maual_barcode_fragment,
            container,
            false
        )
        binding = dataBinding
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.participantOffline?.observe(this, Observer { participantResource ->
            if (participantResource?.status == Status.SUCCESS) {
                ...
                findNavController().navigate(R.id.action_manualScanFragment_to_guideMainFragment, bundle)
            } else if (participantResource?.status == Status.ERROR) {
                val errorDialogFragment = ErrorDialogFragment()
