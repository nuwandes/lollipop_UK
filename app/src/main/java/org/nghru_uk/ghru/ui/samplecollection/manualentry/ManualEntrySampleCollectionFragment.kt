package org.nghru_uk.ghru.ui.samplecollection.manualentry
...
import org.nghru_uk.ghru.databinding.SamplecollectionManualBarcodeFragmentBinding
...
import org.nghru_uk.ghru.ui.samplecollection.scanbarcode.ScanBarcodeViewModel
...
class ManualEntrySampleCollectionFragment : Fragment(), Injectable {

    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory

    var dataBindingComponent: DataBindingComponent = FragmentDataBindingComponent(this)
    var binding by autoCleared<SamplecollectionManualBarcodeFragmentBinding>()
            ...
            StationCheckRxBus.getInstance().toObservable()
                .subscribe({ result ->
                    ...
                    Navigation.findNavController(activity!!, R.id.container)
                        .navigate(R.id.action_manualParticipent_to_verifyIDFragment, bundle)
                }, { error ->
                    print(error)
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<SamplecollectionManualBarcodeFragmentBinding>(
            inflater,
            R.layout.samplecollection_manual_barcode_fragment,
            container,
            false
        )
        binding = dataBinding
...
//                findNavController().navigate(R.id.action_manualParticipent_to_verifyIDFragment, bundle)
//            } else if (participantResource?.status == Status.ERROR) {
//                val errorDialogFragment = ErrorDialogFragment()
