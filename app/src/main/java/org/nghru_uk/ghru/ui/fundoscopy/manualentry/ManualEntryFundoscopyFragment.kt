package org.nghru_uk.ghru.ui.fundoscopy.manualentry
...
import org.nghru_uk.ghru.databinding.FundoscopyManualBarcodeFragmentBinding
import org.nghru_uk.ghru.di.Injectable
import org.nghru_uk.ghru.event.StationCheckRxBus
import org.nghru_uk.ghru.ui.fundoscopy.scanbarcode.ScanBarcodeViewModel
...
class ManualEntryFundoscopyFragment : Fragment(), Injectable {
    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory

    var binding by autoCleared<FundoscopyManualBarcodeFragmentBinding>()
            ...
            StationCheckRxBus.getInstance().toObservable()
                .subscribe({ result ->
                    ...
                    Navigation.findNavController(activity!!, R.id.container)
                        .navigate(R.id.action_manual_to_displayBarcode, bundle)
                }, { error ->
                    print(error)
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<FundoscopyManualBarcodeFragmentBinding>(
            inflater,
            R.layout.fundoscopy_manual_barcode_fragment,
            container,
            false
        )
        binding = dataBinding
...
//                findNavController().navigate(R.id.action_manual_to_displayBarcode, bundle)
//            } else if (participantResource?.status == Status.ERROR) {
//                val errorDialogFragment = ErrorDialogFragment()
