package org.nghru_uk.ghru.ui.samplecollection.scanbarcode
            ...
            StationCheckRxBus.getInstance().toObservable()
                .subscribe({ result ->
                    ...
                    Navigation.findNavController(activity!!, R.id.container)
                        .navigate(R.id.action_scanBarcodeFragment_to_verifyIDFragmentzz, bundle)
                }, { error ->
                    print(error)
            ...
            activity?.runOnUiThread {
                ...
                if (!checkSum.error) {
                    ...
                    if (isNetworkAvailable()) {
                        handler.post({ viewModel.setScreeningId(it.text) })
                    } else {
                        //handler.post({ viewModel.setScreeningIdOffline(it.text) })
                    }
                } else {
...
//                participantRequest?.meta = meta
//                // mFullScannerFragment.stop()
//                // findNavController().navigate(R.id.action_scanBarcodeFragment_to_verifyIDFragmentzz, bundleOf("participant" to participantResource.data))
//                Navigation.findNavController(activity!!, R.id.container).navigate(
//                    R.id.action_scanBarcodeFragment_to_verifyIDFragmentzz,
...
//            val handler = Handler()
//            if (isNetworkAvailable()) {
//                handler.post({ viewModel.setScreeningId(screeningId) })
//            } else {
//                handler.post({ viewModel.setScreeningIdOffline(screeningId) })
//            }
//
//        }
    }
