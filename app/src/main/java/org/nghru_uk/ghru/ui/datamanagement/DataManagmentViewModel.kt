package org.nghru_uk.ghru.ui.datamanagement
...
import org.nghru_uk.ghru.repository.*
...
@Inject constructor(bloodPressureRequestRepository: BloodPressureRequestRepository,
    ...
    // New object for offline syncing -------------- 25-11-2020 ----------------------------------------------------------

    private val _recordBloodPressureMetaRequestNew: MutableLiveData<BloodPressureMetaRequestNew> = MutableLiveData()

    fun setRecordBloodPressureMetaRequestNew(syncRecord : BloodPressureMetaRequestNew)
    {
        if(_recordBloodPressureMetaRequestNew.value == syncRecord)
        {
            return
        }
        _recordBloodPressureMetaRequestNew.value = syncRecord
    }
    var syncRecordBloodPressureMetaRequestNew : LiveData<Resource<ResourceData<BloodPressureMetaRequestNew>>>?= Transformations
        .switchMap(_recordBloodPressureMetaRequestNew) { input ->

            bloodPressureRequestRepository.syncBloodPressureNew(_recordBloodPressureMetaRequestNew?.value!!,_recordBloodPressureMetaRequestNew.value?.body?.screeningId!!)

        }

    var stationBPLocalListNew: LiveData<MutableList<BloodPressureMetaRequestNew>>? = Transformations
        .switchMap(_stationNameBP) { input ->
            bloodPressureRequestRepository.getBloodPressureMetaRequestFromLocalDBNew()
        }
