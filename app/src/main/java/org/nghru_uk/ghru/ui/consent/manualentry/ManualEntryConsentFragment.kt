...
class ManualEntryConsentFragment : Fragment(), Injectable {
    ...
    @Inject
    lateinit var viewModel: ManualEntryConsentViewModel
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.getAssets?.observe(this, Observer { assertsResource ->
            if (assertsResource?.status == Status.SUCCESS) {
                ...
                {
                    val bundle = bundleOf("Participant" to binding.editTextCode.text.toString(), "ConsentCount" to 0)
                ...
                else if (assertsResource.data?.data?.size!! > 0)
                {
                    ...
                    getAssetDialogFragment.arguments = bundleOf("Participant" to binding.editTextCode.text.toString(), "ConsentCount" to assertsResource.data.data.size)
...
//                    {
//                        val bundle = bundleOf("ParticipantRequest" to participantRequest, "ConsentCount" to 2)
//                        findNavController().navigate(R.id.action_manualEntryFragment_to_consentFragment, bundle)
//                    }
//                    else
//                    {
...
//                else
//                {
//                    val bundle = bundleOf("ParticipantRequest" to participantRequest, "ConsentCount" to 1)
//                    findNavController().navigate(R.id.action_manualEntryFragment_to_consentFragment, bundle)
//                }
//
        ...
        if (!checkSum.error) {
            activity?.runOnUiThread({
                ...
                if (isNetworkAvailable()) {
                    ...
                    viewModel.setParticipantForGetAsset(binding.editTextCode.text.toString())
                }
            })
        } else {
