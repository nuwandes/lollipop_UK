package org.nghru_uk.ghru.ui.questionnaire.scanbarcode
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.participant.observe(this, Observer { participantResource ->
            ...
            if (participantResource?.status == Status.SUCCESS) {
                ...
                if (!participantResource.data?.stationStatus!!) {
                    val bundle = bundleOf("ParticipantRequest" to participantRequest, "Questionnaire" to questionnaire, "Meta" to meta)
                    Navigation.findNavController(activity!!, R.id.container)
                        .navigate(R.id.action_ScanBarcodeFragment_to_WebFragment, bundle)
                } else {
        ...
        viewModel.participantOffline?.observe(this, Observer { participantResource ->
            ...
            if (participantResource?.status == Status.SUCCESS) {
                ...
                participantRequest?.meta = meta
                val bundle = bundleOf("ParticipantRequest" to participantRequest, "Questionnaire" to questionnaire)
                //  findNavController().navigate(R.id.action_ScanBarcodeFragment_to_WebFragment, bundle)
                Navigation.findNavController(activity!!, R.id.container)
                    .navigate(R.id.action_ScanBarcodeFragment_to_WebFragment, bundle)
        ...
        binding.buttonManualEntry.singleClick {
            val bundle = bundleOf("meta" to meta, "Questionnaire" to questionnaire )
