...
import org.nghru_uk.ghru.databinding.CheckoutCancelDialogFragmentBinding
...
class CancelDialogFragment : DialogFragment(), Injectable {
    ...
    var binding by autoCleared<CheckoutCancelDialogFragmentBinding>()
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<CheckoutCancelDialogFragmentBinding>(
            inflater,
            R.layout.checkout_cancel_dialog_fragment,
            container,
            false
        )
        binding = dataBinding
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        cancelRequest = CancelRequest(stationType = "checkout")
        binding.radioGroup.setOnCheckedChangeListener { radioGroup, i ->
            ...
            when (radioGroup.checkedRadioButtonId) {
                ...
                R.id.radioButtonWrongParticipantLinked -> cancelRequest.reason =
                    getString(R.string.reason_for_ending_checkout)
                else -> {
...
//            }
            cancelRequest.comment = binding.comment.text.toString()
            cancelRequest.reason = user!!.name + " (" + user!!.id + ")"
            cancelRequest.screeningId = participant?.screeningId!!
            cancelRequest.syncPending = !isNetworkAvailable()
            viewModel.setLogin(participant, cancelRequest)
        }
