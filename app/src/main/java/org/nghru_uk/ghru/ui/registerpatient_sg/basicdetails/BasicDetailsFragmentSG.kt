package org.nghru_uk.ghru.ui.registerpatient_sg.basicdetails
...
import org.nghru_uk.ghru.databinding.BasicDetailsFragmentSgBinding
...
class BasicDetailsFragmentSG : Fragment(), Injectable {

    @Inject
    lateinit var viewModelFactory: ViewModelProvider.Factory

    var binding by autoCleared<BasicDetailsFragmentSgBinding>()

    var dataBindingComponent: DataBindingComponent = FragmentDataBindingComponent(this)

    @Inject
    lateinit var basicDetailsViewModelSG: BasicDetailsViewModelSG
...
//            concentPhoto = arguments?.getString("concentPhotoPath")!!
    ...
    override fun onCreateView(
        ...
        val dataBinding = DataBindingUtil.inflate<BasicDetailsFragmentSgBinding>(
            inflater,
            R.layout.basic_details_fragment_sg,
            container,
            false
        )
        binding = dataBinding
...
//                    if (household?.address?.postcode == null) "" else household?.address?.postcode!!
            ...
            binding.viewModel = basicDetailsViewModelSG
        ...
        binding.nextButton.singleClick {
            ...
            findNavController().navigate(
                R.id.action_BasicDetailFragmentSG_to_reviewFragmentSG,
        ...
        val dateSetListener = DatePickerDialog.OnDateSetListener { view, year, monthOfYear, dayOfMonth ->
            basicDetailsViewModelSG.birthYear = year
            cal.set(Calendar.YEAR, year)
            cal.set(Calendar.MONTH, monthOfYear)
            cal.set(Calendar.DAY_OF_MONTH, dayOfMonth)

            val birthDate: Date = Date(cal.get(Calendar.DAY_OF_MONTH), cal.get(Calendar.YEAR), cal.get(Calendar.MONTH))
            basicDetailsViewModelSG.birthDate.postValue(sdf.format(cal.time))
            basicDetailsViewModelSG.birthDateVal.postValue(birthDate)
            ...
            basicDetailsViewModelSG.age.value = years
...
//        }
        ...
        basicDetailsViewModelSG.setUser("user")
        basicDetailsViewModelSG.user?.observe(this, Observer { userData ->
...
//                if (editText == binding.contactNumberPrimaryEditText)
//                {
//                    validatePrimaryMobile()
//                }

//                if(editText == binding.contactNumberPrimaryEditText && binding.contactNumberPrimaryEditText.length()== 9){
//                    validateNextButton()
//                }
