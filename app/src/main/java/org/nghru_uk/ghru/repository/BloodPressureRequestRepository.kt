...
import org.nghru_uk.ghru.db.*
import org.nghru_uk.ghru.jobs.SyncBloodPresureRequestJob
import org.nghru_uk.ghru.jobs.SyncBloodPresureRequestJobNew
...
class BloodPressureRequestRepository @Inject constructor(
    ...
    private val bloodPressureRequestDaoNew: BloodPresureRequestDaoNew,
    private val bloodPressureItemRequestDaoNew: BloodPresureItemRequestDaoNew,
...
//    }
    ...
    // Same as SG - 100K - BP with new objects  -------------------------25.11.2020 -------------------------------------------------------------------

    fun syncBloodPressureMetaRequestNew(
        bloodPressureMetaRequest: BloodPressureMetaRequestNew,
        particapant: ParticipantRequest

    ): LiveData<Resource<BloodPressureMetaRequestNew>> {
        return object : MyNetworkBoundResource<BloodPressureMetaRequestNew,ResourceData<BloodPressureMetaRequestNew>>(appExecutors) {
            override fun createJob(insertedID: Long) {
                bloodPressureMetaRequest?.body.id = insertedID
                jobManager.addJobInBackground(
                    SyncBloodPresureRequestJobNew(
                        particapant?.screeningId!!,
                        bloodPressureMetaRequest!!
                    )
                )
            ...
            override fun saveDb(): Long {
                ...
                var idMeta = metaNewDao.insert(bloodPressureMetaRequest?.meta)
                bloodPressureMetaRequest?.body?.metaId = idMeta
                var idBloodPressureRequest = bloodPressureRequestDaoNew.insert(bloodPressureMetaRequest?.body)
                for(t in bloodPressureMetaRequest?.body?.bloodPresureRequestList!!)
                {
                    t.bloodPresureRequestId=idBloodPressureRequest
                    bloodPressureItemRequestDaoNew.insert(t)
                }

                return idBloodPressureRequest
            }
            override fun createCall(): LiveData<ApiResponse<ResourceData<BloodPressureMetaRequestNew>>> {
                return nghruService.addBloodPresureRequestNew(particapant.screeningId, bloodPressureMetaRequest)
            }
        }.asLiveData()
    }

    fun getBloodPressureMetaRequestFromLocalDBNew(

    ): MutableLiveData<MutableList<BloodPressureMetaRequestNew>>{
        return object : MyLocalBoundResource<MutableList<BloodPressureMetaRequestNew>>(appExecutors) {
            override fun loadFromDb(): MutableLiveData<MutableList<BloodPressureMetaRequestNew>> {

                var request = MutableLiveData<MutableList<BloodPressureMetaRequestNew>>()
                var requestList : MutableList<BloodPressureMetaRequestNew> = ArrayList()
                var bloodPresureRequestList : List<BloodPresureRequestNew> = bloodPressureRequestDaoNew.getAllBloodPressureRequestsSyncPending()
                for(bloodPresureRequest in bloodPresureRequestList)
                {
                    bloodPresureRequest.bloodPresureRequestList = bloodPressureItemRequestDaoNew.getBloodPressureItemsByBloodPresureRequestID(bloodPresureRequest.id)
                    //var metaRequest : BloodPressureMetaRequest = bloodPressureMetaRequestDao.getBloodPressureMetaRequestByBloodPressureID(bloodPresureRequest.id)
                    var bloodPressureMetaRequest : BloodPressureMetaRequestNew = BloodPressureMetaRequestNew(metaNewDao.getNewMeta(bloodPresureRequest.id),bloodPresureRequest)
                    requestList.add(bloodPressureMetaRequest)
    ...
    fun syncBloodPressureNew(
        bloodPressureMetaRequest: BloodPressureMetaRequestNew,
        screeningId: String
    ): LiveData<Resource<ResourceData<BloodPressureMetaRequestNew>>> {
        return object : SyncNetworkOnlyBcakgroundBoundResource<ResourceData<BloodPressureMetaRequestNew>>(appExecutors) {
            override fun createCall(): LiveData<ApiResponse<ResourceData<BloodPressureMetaRequestNew>>> {
                return nghruService.addBloodPresureRequestNew(screeningId, bloodPressureMetaRequest)
            }

            override fun deleteCall() {
                bloodPressureRequestDaoNew.deleteRequest(bloodPressureMetaRequest.body.id)
            }
        }.asLiveData()
    }
