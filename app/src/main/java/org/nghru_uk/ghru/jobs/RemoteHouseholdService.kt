...
import org.nghru_uk.ghru.db.NGRHUDb
import org.nghru_uk.ghru.di.StringConverterFactory
...
//            L.d("error remote response: " + response.toString())
    ...
    @Throws(IOException::class, RemoteException::class)
    fun addBloodPressuerMetaRequestSyncNew(screeningId: String, bloodPressureMetaRequest: BloodPressureMetaRequestNew) {

        // Remote call can be executed synchronously since the job calling it is already backgrounded.]
        val response = service.addBloodPresureRequestSyncNew(screeningId, bloodPressureMetaRequest).execute()
